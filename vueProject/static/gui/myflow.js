(function($) {
	...
	myflow.config = {
		...
		path : {// 路径转换
			...
			transferType : {
					...
					value : '1',
					editor : function() {
						return new myflow.editors.selectEditor([{name:'正常流转',value:'1'},{name:'条件流转',value:'2'}]);
					}
				},
			taskConditionId : {
				name : 'taskConditionId',
				label : '条件',
				value : '',
				editor : function() {
					return new myflow.editors.selectEditor(conditionUrl);
				}
			},
			formula : {
				name : 'formula',
			...
			convaluetemplate : {
					name : 'convaluetemplate',
					label : '值',
					value : '',
					editor : function() {
						return new myflow.editors.textEditor("1");
					}
				}
			}
		},
	...
	myflow.rect = function(o, r, id) {
		...
		if(_o.text.text != null &&  _o.text.text != "undefined"){
			console.log(_o.text.text);
			//alert(_o.text.text);
			_o.text.text = _o.text.text.replace("</div>","");
		}
		...
		this.moveTo = function(dx,dy) {
			...
			var x = (0 + dx);// -((_ox+dx)%10);
			var y = (0 + dy);// -((_oy+dy)%10);
		...
		this.toJson = function() {
			var tid = 0;
			if(_o.attr != null && _o.attr != 'undefined' && _o.attr.tid != null && _o.attr.tid != 'undefined'){
				tid  = _o.attr.tid;
			}
			...
			var data = "{\"type\":\"" + _o.type + "\",\"text\":{\"text\":\""
					...
					+ Math.round(_rect.attr('height')) + ", \"tid\" : \""+tid+"\" }, \"props\":{";
	...
	myflow.path = function(o, r, from, to) {
		...
		function dot(type, pos, left, right) {
			...
			if (_n && (_t == 'big' || _t == 'small')) {
				...
				var dragStart = function() {// 开始拖动
					...
					myflow.last.dot.x = _ox;
					myflow.last.dot.y = _oy;
					myflow.last.dot.obj = _this;
				};
		...
		_text.drag(function(dx, dy) {
					...
					_ox = _text.attr('x');
					_oy = _text.attr('y');
					myflow.last.x = _textPos.x;
					myflow.last.y = _textPos.y;
				}, function() {
			...
			if (_autoText) {
				if (_to.getId() == src.getId()){
					...
					if(_o.text){
						...
						if(typeof(_o.text.text) != "undefined" && _o.text.text != ''){
		...
		this.toJson = function() {
			...
			var data = "{\"from\":\"" + _from.getId() + "\",\"to\":\"" + _to.getId()
					...
					+ _text.attr('text') + "\"},\"attr\":{\"tid\":\""+tid+"\"},\"textPos\":{\"x\":"
		...
		this.moveTo = function(dx,dy){
			if (!myflow.config.editable)
				return;
			_textPos.x = myflow.last.x;
			_textPos.y = myflow.last.y;
		}
	...
	myflow.props = function(o, r) {
		var _this = this, _pdiv = $('#myflow_props').hide().draggable({
				...
				function() {
					return false;
				}), _tb = _pdiv.find('#_table'), _r = r, _src;

		_tb1= _pdiv.find("#_table1");
		var showTb1 = false;
		var showpropsHandler = function(e, props, src) {
            ...
            _tb.attr("cellspacing",5);
                ...
                for (var k in props) {
                    ...
                    if(row % 2 == 0){
						tempId = new Date().getTime() + '_' + row;
						if(row > 10){
							showTb1 = true;
							_tb1.append('<tr id ="'+tempId+'"></tr>');
						}else{
							_tb.append('<tr id ="'+tempId+'"></tr>');
						}
                    ...
                    $('#'+tempId).append('<th>' + props[k].label + '</th><td><div id="p'
                        + k + '" class="editor"></div></td>');
                    if (props[k].editor)
				...
				if(showTb1){
					_pdiv.find("#ext-info").attr("style","height:30px");
				}else{
					_pdiv.find("#ext-info").attr("style","height:30px;display:none;");
				}

				if(_tb.css('display') == 'none'){
					$('#banner').click();
				}


			if(!myflow.config.display)
				_tb.append('<tr id="myflowDelTR"><th>消息配置</th><td><div id="msg"  onclick="jQuery(document).trigger(\'keydown\',[true,\'msg\']);">配置</div></td><th>删除节点</th><td><div id="del"  onclick="if(confirm(\'确认删除？\'))jQuery(document).trigger(\'keydown\',[true,\'del\']);">删除</div></td></tr>');

		};

		$(_r).bind('showprops', showpropsHandler);
		//开始进入触发一次点击----add by cj
...
//						}]);
	...
	// 属性编辑器
	myflow.editors = {
		textEditor : function(arg) {
			var _props, _k, _div, _src, _r,_text;
			this.init = function(props, k, div, src, r) {
				_props = props;
				_k = k;
				_div = div;
				_src = src;
				if(k == 'text' && _src.val(k) == ''){
					_text = _src.text();
				}else{
					_text = _src.val(k);
				}
				_r = r;
				$('<input  style=""/>').val(_text).change(
						function() {
	...
	myflow.init = function(c, o) {
	 ...
	 /*    $('#myflow').css('width',_w);
		 ...
		 */
		$(document).keydown(function(arg,byButton,type) {
			if (!myflow.config.editable)
				return;
			if(byButton){
				if(typeof(type) != "undefined" && type == 'msg'){
					...
					if (c) {
						if (c.getId().substring(0, 4) == 'rect') {
							//console.log(c.getTid());
							window.location.href = getRootPath() + "#/wf/taskMsgSet/?id=" + c.getTid();
						}
					}
				}
			}
			...
			if (arg.keyCode == 46||byButton) {
				if(typeof(type) != "undefined" && type == 'del'){
					var c = $(_r).data('currNode');
					if (c) {
						if (c.getId().substring(0, 4) == 'rect') {
				...
				//alert(arg.keyCode);
			//撤销操作 extends
			if(arg.ctrlKey && String.fromCharCode(arg.keyCode) == "Z"){
				var c = $(_r).data('currNode');
				var unlink = true;
				if (c) {
					if (c.getId().substring(0, 4) == 'rect') {
						c.moveTo(myflow.last.x,myflow.last.y);
						c.refresh();
					} else if (c.getId().substring(0, 4) == 'path') {
						unlink = false;
						c.moveTo(myflow.last.x,myflow.last.y);
						if(myflow.last.dot.x != 0){
							if(myflow.last.dot.obj)
								myflow.last.dot.obj.moveTo(myflow.last.dot.x,myflow.last.dot.y);
							myflow.last.dot.x = 0;
							myflow.last.dot.y = 0;
						}
						c.refresh();
		...
		var removeHandler = function(e, src) {
			...
			if (src.getId().substring(0, 4) == 'rect') {
				...
				src.remove();
				//删除触发ajax 删除节点 提高效率  add by cj
				if(src.getTid() != null && src.getTid() != 'undefined' && src.getTid() != "0"){
					//alert(src.getTid());
					$.ajax({
						 /*  data : {
									"id": src.getTid()
						   ...
						   success: function(data){
								if(data != null && data != "undefined"){
									...
									//var dat = JSON.parse(data);  //	JSON字符串转为JSON对象
				...
				//delPathUrl
				if(src.getTid() != null && src.getTid() != 'undefined' && src.getTid() != "0"){
					//alert(src.getTid().value);
					$.ajax({
						 /*  data : {
									"id": src.getTid()
								  },*/
						   type: "GET",
						   url: delPathUrl + '/' + src.getTid(),
						   beforeSend:function(request){
							request.setRequestHeader("Auth-User-Token",token);
							},
						   success: function(data){
		...
		if (myflow.config.editable) {
			...
			$(c).droppable({
						...
						drop : function(event, ui) {
							...
							var temp = ui.helper.context.innerHTML.trim();
			...
			$('#myflow_save').click(function() {// 保存方法
						var data = '{\"states\":{';
						...
						var props = myflow.config.props.props;
		...
		//add by cj 添加url 初始化方式
		if(o.location != null && o.location != null){
			$.ajax({
				   type: "GET",
				   url: o.location,
				   beforeSend:function(request){
				   ...
				   success: function(data){
							...
							if(data.props){
								if(data.props.props){
									var __props = myflow.config.props.props;
									if(data.props.props.name){
										__props.name.value = data.props.props.name.value;
									}
									if(data.props.props.key){
										__props.key.value = data.props.props.key.value;
									}
									if(data.props.props.desc){
										__props.desc.value = data.props.props.desc.value;
									}
								}
							}
